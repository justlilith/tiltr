...
update msg model =
    case msg of
        ...
        ArtistClicked artistname ->
            ( { model | searchString = "@" ++ artistname }
              --backend expects an "a"
...
decodeTweetList : Decoder (List TweetPost)



-- Return only good JSON
...
tweetEntitiesDecoder =
    Decode.index 0
        -- The medias object is always top-level, or, at index 0
        ...
        I'm leaving this in for future development. --}
            ...
            , div [ id "center" ]
                [ -- text model.searchString
                  {- Debug.log "" -}
                  viewTweetList model.searchResponseList
                ]
            ]
        ]
    }
...
viewLink hashtag =
    li []
        [ a
            [ href hashtag
            , onClick
                (Url.percentEncode hashtag
                    -- A hash (#) isn't sendable to the backend
                    |> HashtagClicked
                )
            ]
            [ text hashtag ]
